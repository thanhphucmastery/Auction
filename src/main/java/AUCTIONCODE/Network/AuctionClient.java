...
public class AuctionClient {
    ...
    public synchronized String sendRaw(String request) {
        if (!isConnected()) {
            return rememberResponse("ERROR:Client is not connected");
        }
        try {
            out.println(request);
            if (out.checkError()) {
                clearConnectionState();
                return rememberResponse("ERROR:Failed to send request");
            }
            String response = in.readLine();
            if (response == null) {
                clearConnectionState();
                return rememberResponse("ERROR:Server closed the connection");
            }
            return rememberResponse(response);
        } catch (IOException e) {
            clearConnectionState();
            return rememberResponse("ERROR:" + e.getMessage());
        }
    }
    ...
    public boolean resetPassword(String username, String email, String newPassword) {
        String response = sendRaw("RESET_PASSWORD:" + username + ":" + email + ":" + newPassword);
        return response != null && response.startsWith("OK");
    }
    ...
    public boolean withdraw(double amount) {
        String response = sendRaw("WITHDRAW:" + sessionId + ":" + amount);
        return response != null && response.startsWith("OK");
    }
    ...
    public String getAuctionDetail(String auctionId) {
        String response = sendRaw("GET_AUCTION_DETAIL:" + sessionId + ":" + auctionId);
        if (response != null && response.startsWith("OK:")) {
            return response.substring(3);
        }
        return null;
    ...
    public String getTransactionHistory() {
        String response = sendRaw("GET_TRANSACTION_HISTORY:" + sessionId);
        if (response != null && response.startsWith("OK:")) {
            return response.substring(3);
        }
        return null;
    ...
    public String getMyItems() {
        String response = sendRaw("GET_MY_ITEMS:" + sessionId);
        if (response != null && response.startsWith("OK:")) {
            return response.substring(3);
        }
        return null;
    ...
    public String addItem(String type, String name, String description, String extra1, String extra2, String extra3, String imagePath) {
        String response = sendRaw("ADD_ITEM:" + sessionId + ":" + encodeField(type) + ":" + encodeField(name) + ":"
                + encodeField(description) + ":" + encodeField(extra1) + ":" + encodeField(extra2) + ":"
                + encodeField(extra3) + ":" + encodeField(imagePath));
        if (response != null && response.startsWith("OK:")) {
            return response.substring(3);
        }
        return null;
    ...
    public boolean updateItem(String itemId, String type, String name, String description, String extra1, String extra2, String extra3, String imagePath) {
        String response = sendRaw("UPDATE_ITEM:" + sessionId + ":" + encodeField(itemId) + ":" + encodeField(type) + ":"
                + encodeField(name) + ":" + encodeField(description) + ":" + encodeField(extra1) + ":"
                + encodeField(extra2) + ":" + encodeField(extra3) + ":" + encodeField(imagePath));
        return response != null && response.startsWith("OK");
    }

    public boolean deleteItem(String itemId) {
        String response = sendRaw("DELETE_ITEM:" + sessionId + ":" + encodeField(itemId));
        return response != null && response.startsWith("OK");
    }

    public String getProfile() {
        String response = sendRaw("GET_PROFILE:" + sessionId);
        if (response != null && response.startsWith("OK:")) {
            return response.substring(3);
        }
        return null;
    ...
    public boolean updateProfile(String fullName, String phone, String email, String address) {
        String response = sendRaw("UPDATE_PROFILE:" + sessionId + ":" + fullName + ":" + phone + ":" + email + ":" + address);
        return response != null && response.startsWith("OK");
    }
    ...
    public boolean adminEndAuction(String auctionId) {
        String response = sendRaw("ADMIN_END_AUCTION:" + sessionId + ":" + auctionId);
        return response != null && response.startsWith("OK");
    }

    public boolean adminExtendAuction(String auctionId, long minutes) {
        String response = sendRaw("ADMIN_EXTEND_AUCTION:" + sessionId + ":" + auctionId + ":" + minutes);
        return response != null && response.startsWith("OK");
    }

    public boolean adminDeleteAuction(String auctionId) {
        String response = sendRaw("ADMIN_DELETE_AUCTION:" + sessionId + ":" + auctionId);
        return response != null && response.startsWith("OK");
    }

    public boolean adminDeleteUser(String userId) {
        String response = sendRaw("ADMIN_DELETE_USER:" + sessionId + ":" + userId);
        return response != null && response.startsWith("OK");
    }
    ...
    public String createAuctionFromItemAndReturnId(String itemId, double startPrice, double stepPrice,
                                                   long startDelayMinutes, long durationMinutes) {
        String response = sendRaw("CREATE_AUCTION_FROM_ITEM:" + sessionId + ":" + encodeField(itemId) + ":"
                + startPrice + ":" + stepPrice + ":" + startDelayMinutes + ":" + durationMinutes);
        if (response != null && response.startsWith("OK:")) {
            return response.substring(3);
        }
        return null;
    ...
    public String getLastError() {
        if (lastResponse != null && lastResponse.startsWith("ERROR:")) {
            return lastResponse.substring("ERROR:".length());
        }
        return null;
    }
    ...
    private String rememberResponse(String response) {
    ...
    private static void runMenu(AuctionClient client, Scanner scanner) {
        ...
        while (running && client.isConnected()) {
            ...
            switch (choice) {
                ...
                case "4" -> System.out.println("Use the JavaFX warehouse flow to create auctions from registered items.");
    ...
    private static void printMenu(AuctionClient client) {
        ...
        System.out.println("4. Create auction from warehouse item (JavaFX only)");
