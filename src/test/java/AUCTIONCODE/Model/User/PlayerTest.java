...
class PlayerTest {
    ...
    @Test
    void testDeepCopy_ShouldReturnNewIndependentPlayer() {
        Player original = new Player("player1", userInfo, "pass", "PL001", 250.0, "PLAYER");
        User copy = original.deepCopy();

        assertTrue(copy instanceof Player);
        Player clonedPlayer = (Player) copy;

        assertNotSame(original, clonedPlayer);
        assertNotSame(original.getUserInformation(), clonedPlayer.getUserInformation());

        assertEquals(original.getPlayerBalance(), clonedPlayer.getPlayerBalance());
        assertEquals(original.getUserName(), clonedPlayer.getUserName());
    }
